...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run_dreamx.ps1" "%LOG%" "%PY%" "src\drive_anchorweave.py" "--gt-root" "%GT_ROOT%" "--test-root" "%MIND_TESTS%" "--model-name" "%MODEL_NAME%" "--repo" "%ANCHORWEAVE_REPO%" "--py" "%ANCHORWEAVE_PY%" "--perspective" "1st_data" %*
