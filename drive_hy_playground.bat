...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run_dreamx.ps1" "%LOG%" "%PY%" "src\drive_hy_playground.py" "--gt-root" "%GT_ROOT%" "--test-root" "%MIND_TESTS%" "--model-name" "%MODEL_NAME%" "--repo" "%HY_PLAYGROUND_REPO%" "--py" "%HY_PLAYGROUND_PY%" "--perspective" "1st_data" %*
