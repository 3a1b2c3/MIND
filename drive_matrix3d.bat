...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run_dreamx.ps1" "%LOG%" "%PY%" "src\drive_matrix3d.py" "--gt-root" "%GT_ROOT%" "--test-root" "%MIND_TESTS%" "--model-name" "%MODEL_NAME%" "--repo" "%MATRIX3D_REPO%" "--py" "%MATRIX3D_PY%" "--fps" "%MIND_FPS%" "--perspective" "1st_data" %*
